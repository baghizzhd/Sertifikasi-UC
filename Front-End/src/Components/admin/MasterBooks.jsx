...
const MasterBuku = () => {
  ...
  const handleTicket = async (searchValue, token) => {
    ...
    await axiosJWT.get(`http://localhost:8000/fetchbooks?search=${searchValue}`,
      {
        headers: {
          Authorization: `Bearer ${token}`
        },
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        const response = await axiosJWT.delete(`http://localhost:8000/deletebook/${id}`, {
          headers: {
    ...
    <main className={navVisible ? 'space-toggle' : ''}>
      ...
      <div className='content'>
       <div className='flex items-baseline headline'>
          <h1 className='font-semibold text-ticket'>Books</h1>
              ...
              <div className='whiteadmin-box mt-1 shadow-lg'>
                  <div className='flex mt-2 px-4'>
                      ...
                      <div className='flex gap-2 ml-[auto]'>
                        <div  onClick={() => navigate('/books-new')}  className='ml-6 flex  items-center justify-center rounded-md mt-2 text-base font-semibold px-4 w-32 text-white cursor-pointer btn-NewTicket'>
                          Add Book
                        </div>
                      </div>     
                  </div>
                            ...
                            books.map((books, index) => (
                              <tr key={books.TicketId}>
                                  ...
                                  <td className="px-1 py-1 text-sm text-center">
                                    <p className="px-1 py-1 text-sm text-center">{books.author}</p>
                                  </td>
                                  <td className='px-1 py-1 text-sm text-center'>
                                      {books.publication}
                                  </td>
                                  ...
                                  <td className='px-1 py-1 flex items-center justify-center'>
                                      <span className={`w-24 px-1 py-1 items-center text-sm justify-center flex font-semibold rounded-full ${
                                          books.status === 'Available' ? "bg-opacity-50 text-green-800 bg-green-200" : "bg-opacity-50 text-orange-800 bg-orange-200"
                                      }`}>
                                          {books.status}
                                      </span>
                                  </td>
