...
const MasterBuku = () => {
  ...
  const handleTicket = async (searchValue, token) => {
    ...
    await axiosJWT.get(`http://localhost:8000/fetchusers?search=${searchValue}`,
      {
        headers: {
          Authorization: `Bearer ${token}`
        },
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        const response = await axiosJWT.delete(`http://localhost:8000/deleteuser/${id}`, {
          headers: {
    ...
    <main className={navVisible ? 'space-toggle' : ''}>
      ...
      <div className='content'>
       <div className='flex items-baseline headline'>
          <h1 className='font-semibold text-ticket'>Users</h1>
